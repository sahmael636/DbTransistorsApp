...
public partial class TransistorListPage : ContentPage, IQueryAttributable
{
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            // Shell entrega los parámetros antes de mostrar la página. Esperar aquí evita
            // construir encabezados o plantillas con el ViewModel aún sin inicializar.
            await _initializationTask;
            await vm.OnAppearingAsync();

            if (!_headerBuilt && vm.HeaderFields.Count > 0)
            {
                BuildHeader(vm);
                _headerBuilt = true;
            }

            if (!_templateBuilt && vm.HeaderFields.Count > 0)
            {
                BuildItemTemplate();
                _templateBuilt = true;
            }
            ...
            if (BindingContext is TransistorListViewModel vm)
            {
                ...
                if (query.TryGetValue("TableType", out var tableObj) || query.TryGetValue("tableType", out tableObj))
                {
                    ...
                    if (!string.IsNullOrEmpty(s) && Enum.TryParse<TableType>(s, true, out var tt))
                    {
                        ...
                        _initializationTask = MainThread.InvokeOnMainThreadAsync(() => vm.InitializeAsync(tt));
                    }
                }
            }
        }
